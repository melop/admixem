#Simulates change in allele frequency as a result of two locus selection based on Karlin 1975, with drift implemented by multinomial sampling
...
# new haplotype frequencies
	y<-{}
	l<-(as.numeric(round(x[1]*(wstar[1]/w_mean)-(0.5*D*w(s)[1,4]/w_mean),10)))
	m<-(as.numeric(round(x[2]*(wstar[2]/w_mean)+(0.5*D*w(s)[1,4]/w_mean),10)))
	n<-(as.numeric(round(x[3]*(wstar[3]/w_mean)+(0.5*D*w(s)[1,4]/w_mean),10)))
	o<-(as.numeric(round(x[4]*(wstar[4]/w_mean)-(0.5*D*w(s)[1,4]/w_mean),10)))
	
    #perform multinomial sampling to simulate drift
	freqs_sample<-rmultinom(1,1000,c(l,m,n,o))
	
	l<-freqs_sample[1,]/(sum(freqs_sample))
	m<-freqs_sample[2,]/(sum(freqs_sample))
	n<-freqs_sample[3,]/(sum(freqs_sample))
	o<-freqs_sample[4,]/(sum(freqs_sample))
	
	y<-c(l,m,n,o)
	
	x<-y
	
	X<-rbind(X,as.numeric(y)) # store two locus genotype frequencies
} # for numreps
